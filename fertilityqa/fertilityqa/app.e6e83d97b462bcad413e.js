  ...
  _createClass(FertilityD3, [{
    ...
    value: function init() {
      ...
      d3.json('./data/1980.json', function (rawdata) {
        ...
        var bounds = _this.getBounds(data, 1);
        debug(data);
        debug('bounds', bounds);
        ...
        d3.select('svg g.chart').append('text').attr({ 'id': 'xLabel', 'x': 400, 'y': 670, 'text-anchor': 'middle' }).text(xAxis);
        d3.select('svg g.chart').append('text').attr('transform', 'translate(-60, 330)rotate(-90)').attr({ 'id': 'yLabel', 'text-anchor': 'middle' }).text(yAxis);
        xScale = d3.scale.linear().domain([bounds['rate1'].min, bounds['rate1'].max]).range([20, 780]);
        yScale = d3.scale.linear().domain([bounds['rate2'].min, bounds['rate2'].max]).range([600, 100]);
        ...
        d3.select('svg g.chart').selectAll('circle').data(data).enter().append('circle').attr('cx', function (d) {
          return isNaN(d['rate1']) ? d3.select(this).attr('cx') : xScale(d[xAxis]);
        }).attr('cy', function (d) {
          return isNaN(d['rate2']) ? d3.select(this).attr('cy') : yScale(d[yAxis]);

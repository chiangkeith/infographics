...
var _comm = __webpack_require__(56);
...
var debug = __webpack_require__(11)('FERTILITY:DEFAULT');
var debugScroll = __webpack_require__(11)('FERTILITY:SCROLL');
...
var Article = function () {
  ...
  _createClass(Article, [{
    ...
    value: function init() {
      ...
      (0, _comm.renderChart)(document.querySelector('.chart-container.a2m08 .hichart'), 'm08');
      ...
      (0, _comm.renderChart)(document.querySelector('.chart-container.a4m18 .hichart'), 'm18');
      ...
      Promise.all([this.preCalc(), this.setScrollManager()]).then(function () {
        debug('INIT FUNISHED');
        _this7.d31 = new _d.FertilityD3();
        _this7.d31.init('#chart-d3-1');
        _this7.d32 = new _d.FertilityD3();
        _this7.d32.init('#chart-d3-2');
        _this7.d33 = new _d.FertilityD3();
        _this7.d33.init('#chart-d3-3');
        _this7.d34 = new _d.FertilityD3();
        _this7.d34.init('#chart-d3-4');
        _this7.d35 = new _d.FertilityD3();
        _this7.d35.init('#chart-d3-5');
        _this7.d36 = new _d.FertilityD3();
        _this7.d36.init('#chart-d3-6');
      });
    }
  }, {
    key: 'preCalc',
    ...
    value: function resetScrollManager() {
      ...
      return Promise.all([new Promise(function (resolve) {
        windoe.removeEventListener('scroll', _this11.scrollHandler);
        windoe.removeEventListener('scroll', _this11.scrollHandlerForFix);
        resolve();
...
var debug = __webpack_require__(11)('d3');
...
var FertilityD3 = exports.FertilityD3 = function () {
  ...
  _createClass(FertilityD3, [{
    key: 'init',
    value: function init(eleSelector) {
      var _this = this;
      ...
      d3.json('./data/1980.json', function (rawdata) {
        ...
        _this.data = rawdata.children;
        _this.bounds = _this.getBounds(_this.data, 1);
        debug(_this.data);
        debug('bounds', _this.bounds);

        var svg = d3.select(eleSelector).append('svg').attr('width', 1000).attr('height', 640);

        svg.append('g').classed('chart', true).attr('transform', 'translate(80, -60)');
        d3.select('svg g.chart').append('text').attr({ 'id': 'countryLabel', 'x': 0, 'y': 170 }).style({ 'font-size': '80px', 'font-weight': 'bold', 'fill': '#ddd' });
        d3.select('svg g.chart').append('line').attr('id', 'bestfit');
        d3.select('svg g.chart').append('text').attr({ 'id': 'xLabel', 'x': 400, 'y': 670, 'text-anchor': 'middle' }).text('生育率');
        d3.select('svg g.chart').append('text').attr('transform', 'translate(-60, 330)rotate(-90)').attr({ 'id': 'yLabel', 'text-anchor': 'middle' }).text('女性勞参率');

        _this.updateScales();
        var pointColour = d3.scale.category20b();
        d3.select('svg g.chart').selectAll('circle').data(_this.data).enter().append('circle').attr('cx', function (d) {
          return isNaN(d[_this.xAxis]) ? d3.select(_this).attr('cx') : _this.xScale(d[_this.xAxis]);
        }).attr('cy', function (d) {
          return isNaN(d[_this.yAxis]) ? d3.select(_this).attr('cy') : _this.yScale(d[_this.yAxis]);
        }).attr('fill', function (d, i) {
          return pointColour(i);
        }).style('cursor', 'pointer').on('mouseover', function (d) {
          d3.select('svg g.chart #countryLabel').text(d.name).transition().style('opacity', 1);
        }).on('mouseout', function (d) {
          d3.select('svg g.chart #countryLabel').transition().duration(1500).style('opacity', 0);
        });
        _this.updateChart(true);
        d3.select('svg g.chart').append("g").attr('transform', 'translate(0, 630)').attr('id', 'xAxis').call(_this.makeXAxis);
        d3.select('svg g.chart').append("g").attr('id', 'yAxis').attr('transform', 'translate(-10, 0)').call(_this.makeYAxis);
      });
    }
  }, {
    key: 'updateScales',
    value: function updateScales() {
      this.xScale = d3.scale.linear().domain([this.bounds[this.xAxis].min, this.bounds[this.xAxis].max]).range([20, 780]);
      this.yScale = d3.scale.linear().domain([this.bounds[this.yAxis].min, this.bounds[this.yAxis].max]).range([600, 100]);
    }
  }, {
    key: 'updateChart',
    value: function updateChart(init) {
      ...
      this.updateScales();
      d3.select('svg g.chart').selectAll('circle').transition().duration(500).ease('quad-out').attr('cx', function (d) {
        return isNaN(d[_this2.xAxis]) ? d3.select(_this2).attr('cx') : _this2.xScale(d[_this2.xAxis]);
      }).attr('cy', function (d) {
        return isNaN(d[_this2.yAxis]) ? d3.select(_this2).attr('cy') : _this2.yScale(d[_this2.yAxis]);
      }).attr('r', function (d) {
      ...
      d3.select('#bestfit').style('opacity', 0).attr({ 'x1': this.xScale(x1), 'y1': this.yScale(y1), 'x2': this.xScale(x2), 'y2': this.yScale(y2) }).transition().duration(1500).style('opacity', 1);
    }
  }, {
    key: 'getBounds',
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(100), __webpack_require__(101)(module)))
